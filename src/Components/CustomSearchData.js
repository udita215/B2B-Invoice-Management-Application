...
export function SearchGridData({ val, selectedRows, selectRow }) {
    ...
    const search = async () => {
      try {
        let response = await axios.get(
          "http://localhost:8080/HRC_INTERNSHIP/SearchServlet",
          {
            params: {
              cust_number: val,
            },
    ...
    <>
      <div style={{ height: 370, width: "100%" }}>
        <DataGrid
          ...
          //   selectRow(data.filter((e) => ids.indexOf(e.sl_no) > -1));
          // }}
          //pagination
        />
      </div>
